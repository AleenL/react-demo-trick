...
// *.+(js|css) : 匹配项目根目录下，所有后缀名为js或者css的文件

// 流 stream 管道 pipe 管道
// 如果想在读取流和写入流的时候做完全的控制，可以使用数据事件。但对于单纯的文件复制来说读取流和写入流可以通过管道来

// * 复制目录中所有的文件包括子文件
// * @src param{ String } 需要复制的目录 例如 images 或者 ./images/
// * @dst param { String } 复制到指定的目录 例如 images images/
    ...
    if(fs.existsSync(src)){
        fs.readdir(src,function(err,files){
            ...
            files.forEach(function(filename){
                ...
                //url + '/' + filename 不能用/直接连接，Unix系统是"/" , Windows系统是"\"
